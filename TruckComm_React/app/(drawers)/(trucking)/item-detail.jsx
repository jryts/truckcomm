...
export default function ItemDetail() {
    ...
    const getValue = (header) => {
        if (header == headers[0]) {
            ...
            return (modalItem.QTY !== undefined ? 'x ' + modalItem.QTY : 0)
        } else if (header == headers[4]) {
            return (modalItem.ActQty !== undefined ? 'x ' + modalItem.ActQty : 0)
        } else if (header == headers[5]) {
            return (modalItem.UNIT_WEIGHT !== undefined ? modalItem.UNIT_WEIGHT + ' (kg)' : 0)
        } else if (header == headers[6]) {
            return (modalItem.TOTAL_WEIGHT !== undefined ? modalItem.TOTAL_WEIGHT + ' (kg)' : 0)
        } else if (header == headers[7]) {
            return (modalItem.LENGTH !== undefined ? modalItem.LENGTH + ' (cm)' : 0)
        } else if (header == headers[8]) {
            return (modalItem.WIDTH !== undefined ? modalItem.WIDTH + ' (cm)' : 0)
        } else if (header == headers[9]) {
            return (modalItem.HEIGHT !== undefined ? modalItem.HEIGHT + ' (cm)' : 0)
        } else if (header == headers[10]) {
            return (modalItem.TOTAL_VOLUME !== undefined ? roundUp(modalItem.TOTAL_VOLUME, 4) + ' (cbm)' : 0)
        ...
        <View style={{ flex: 1 }}>
            <View style={styles.container}>
                <View style={{ margin: 10 }}>
                    <Text variant="titleMedium">{i18n.t('cargo_description')}: </Text>
                    <Text>{(items.length) > 0 ? items[0].CARGO_DESC : ''}</Text>
                </View>
                <View style={{ margin: 10 }}>
                    <Text variant="titleMedium">{i18n.t('remark')}: </Text>
                    <Text>{(items.length) > 0 ? items[0].REMARKS : ''}</Text>
                </View>
            </View>
            <View style={{ flex: 4 }}>
                <DataTable>
                    ...
                    {items.map((item) => (
                        <DataTable.Row key={item.Marking} onPress={() => {
                            ...
                            <DataTable.Cell>{(item.Marking !== '') ? item.Marking : '-'}</DataTable.Cell>
                            <DataTable.Cell>{(item.UOM !== '') ? item.UOM : '-'}</DataTable.Cell>
                            <DataTable.Cell>{(item.QTY !== undefined) ? item.QTY : 0}</DataTable.Cell>
                            <DataTable.Cell>{(item.TOTAL_VOLUME !== undefined) ? roundUp(item.TOTAL_VOLUME, 4) : 0}</DataTable.Cell>
                            <DataTable.Cell>{(item.TOTAL_WEIGHT !== undefined) ? item.TOTAL_WEIGHT : 0}</DataTable.Cell>
                        </DataTable.Row>
                    ))}
                </DataTable>
                <Portal>
